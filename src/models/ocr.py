...
def main():
    ...
    # Load network
    print(modelDetector)
    detector = cv.dnn.readNet(str(modelDetector))
    ...
    while cv.waitKey(1) < 0:
        ...
        #
        # cv.namedWindow(kWinName, cv.CV_WINDOW_AUTOSIZE)
        ...
        for pos, i in enumerate(indices):
            ...
            if modelRecognition:
                ...
                # print(pos, i, wordRecognized)
                recognized[pos] = {
                    "index": int(i),
                    "text": wordRecognized,
                    "position": (int(vertices[1][0]), int(vertices[1][1])),
                }
                cv.putText(
                    frame,
                    wordRecognized,
